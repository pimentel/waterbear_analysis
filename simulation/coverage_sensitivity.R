...
process_subgroup = function(metadata, p_gene_effects, n_total_genes, bin_configuration,
  ...
  if (!waterbear_only) {
    ...
    s_maude_res = maude_results = NULL
    ...
    all_res = s_res
    if (debug) {
      return(list(all_res = all_res, waterbear = res))
    }
  }
...
metadata = dplyr::filter(metadata, effect_size_type == 'equalmixture',
  n_total_genes == '1000', n_replicates == '3')
metadata = dplyr::filter(metadata, n_guides_per_target == 4)

metadata_filter = filter(metadata, n_cells <= 1000 * 4100)

p_10 = process_subgroup(metadata_filter, p_gene_effects, n_total_genes, bin_configuration, n_replicates)
...
saveRDS(p_10, 'p_10_coverage_sensitivity.rds')

p_10 = readRDS('p_10_coverage_sensitivity.rds')
...
tmp_10 = mutate(tmp_10, coverage = as.integer(n_cells) / (4100))
tmp_10 = dplyr::filter(tmp_10, method == 'waterbear', coverage <= 1000)

labels = distinct(select(ungroup(tmp_10), coverage))
labels = mutate(labels, text = paste0(coverage, 'x4'))
labels = arrange(labels, coverage)
labels = mutate(labels,
  # x = c(0.025, 0.025, 0.025, 0.05, 0.07, 0.085, 0.11, NA),
  # y = c(0.45, 0.60, 0.75, 0.83, 0.88, 0.94, 0.96, NA))
  x = c(0.025, 0.025, 0.025, 0.05, 0.07),
  y = c(0.45, 0.60, 0.75, 0.83, 0.88))
labels = labels[complete.cases(labels), ]
p = ggplot(
  # dplyr::filter(tmp_10, method == 'waterbear', coverage < 50000),
  # dplyr::filter(tmp_10, method == 'waterbear', coverage <= 1000),
  tmp_10,
  aes(mean_fdr, mean_sensitivity, color = factor(coverage)), alpha = 0.2)
p = p + geom_path(size = 1.5)
...
# p = p + xlim(0, 0.25)
p = p + xlim(0, 0.15)
p = p + geom_text(aes(x, y, color = factor(coverage), label = text), data = labels, size = 10)
p = p + xlab('True false discovery rate')
p = p + ylab('Sensitivity')
p = p + scale_color_manual(values = cbb, labels = labels$text)
p = p + theme_cowplot(25)
p = p + theme(legend.position="none")
p1 = p
save_plot(paste0('tmp', base, '.pdf'), p1, base_height = 7)
...
waterbear_em = equal_mixture_sensitivity(p_10$waterbear, 'waterbear')
waterbear_em = mutate(waterbear_em, coverage = as.integer(n_cells) / 4100)
waterbear_em = mutate(waterbear_em, type = factor(type, c('difficult', 'moderate', 'easy')))

p = ggplot(waterbear_em %>% filter(type != 'none', coverage == 50),
  aes(as.factor(type), sensitivity, fill = type))
p = p + geom_boxplot()
p = p + theme(legend.position="none")
p = p + xlab('')
save_plot('tmp_sensitivity.pdf', p)
...
# XXX: update this plot
waterbear_em = equal_mixture_sensitivity(p_10$waterbear, 'waterbear')
waterbear_em = mutate(waterbear_em, coverage = as.integer(n_cells) / 4100)
waterbear_em = mutate(waterbear_em, type = factor(type, c('easy', 'moderate', 'difficult')))

p = ggplot(
  filter(waterbear_em, coverage <= 1000 & !is.na(type)), aes(factor(coverage), sensitivity, fill = factor(coverage)))
p = p + geom_boxplot()
p = p + facet_wrap(~type)
p = p + scale_fill_manual(values = cbb)
p = p + theme(legend.position = 'none',
  axis.text.x = element_text(angle = 25, vjust = 0.5, hjust=1))
p = p + xlab('coverage per guide')
save_plot('sensitivity_by_effect_size_type.pdf', p)
...
waterbear_em = equal_mixture_sensitivity(p_10$waterbear, 'waterbear')
# waterbear_em = equal_mixture_sensitivity(p_10, 'waterbear')
waterbear_em = mutate(waterbear_em, coverage = as.integer(n_cells) / 4100)
waterbear_em = mutate(waterbear_em, type = factor(type, c('easy', 'moderate', 'difficult')))

z_95 = abs(qnorm(0.025))
tmp_summary = group_by(waterbear_em, coverage, type) %>%
  summarize(
...
tmp_summary = filter(tmp_summary, coverage <= 1000, !is.na(type))

p = ggplot(tmp_summary, aes(factor(coverage), m_sensitivity, color = factor(coverage)))
p = p + geom_point()
...
p = p + theme(legend.position = 'none',
  axis.text.x = element_text(angle = 25, vjust = 0.5, hjust=1))
p = p + xlab('Coverage per guide x 4 guides')
p = p + ylab('Sensitivity')
save_plot('tmp.pdf', p)
...
saveRDS(p_10, 'p_10_coverage_bin4.rds')
