...
metadata = dplyr::mutate(metadata,
  coverage_per_guide = as.integer(n_cells) /
    (as.integer(n_guides_per_target) * as.integer(n_total_genes) + as.integer(n_control_guides)))

metadata = dplyr::mutate(metadata,
  coverage_per_gene = as.integer(n_guides_per_target) * coverage_per_guide)
metadata = dplyr::filter(metadata, n_mcmc_samples == 20000)
metadata = dplyr::filter(metadata, n_guides_per_target != '3')
metadata = dplyr::filter(metadata, coverage_per_gene <= 4000)
...
process_subgroup = function(metadata, p_gene_effects, n_total_genes, bin_configuration,
  ...
  coverage_mapping = dplyr::select(metadata_filter,
    n_cells, n_guides_per_target, coverage_per_gene, coverage_per_guide)
  coverage_mapping = distinct(coverage_mapping)

  s_res = group_by(res, effect_size_type, n_cells, n_guides_per_target, rank)
  ...
  #   coverage = as.numeric(n_cells) / (
  #     as.integer(n_total_genes) * as.integer(n_guides_per_target) + as.integer(n_control_guides)))
  s_res = left_join(s_res, coverage_mapping, by = c('n_cells', 'n_guides_per_target'))
...
p = ggplot(
  p_10,
  aes(mean_fdr, mean_sensitivity, color = factor(coverage_per_gene),
    linetype = factor()), alpha = 0.2)
