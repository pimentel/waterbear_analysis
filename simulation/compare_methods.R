...
save_plot('img/average_fdr_compare.pdf', p)

p = ggplot(tmp, aes(as.factor(coverage), fdr, fill = method))
p = p + geom_boxplot()
save_plot('img/tmp.pdf', p)

epsilon = 0.001
fdr_10 = dplyr::filter(wb_res, mean_fdr < 0.10 + epsilon, 0.10 - epsilon < mean_fdr)


# given a fixed FDR, compute the sensitivity and true fdr
...
fdr_05 = dplyr::filter(all_methods, fdr < 0.05 + epsilon, 0.05 - epsilon < fdr)
...
save_plot('img/all_methods_fdr_10.pdf', p)

p = ggplot(dplyr::filter(efdr_10, coverage <= 5000),
  aes(as.factor(coverage), fdr, fill = method))
p = p + geom_boxplot()
# p = p + facet_wrap(~n_cells)
save_plot('img/all_methods_estimated_fdr_10.pdf', p)

p = ggplot(dplyr::filter(fdr_05, coverage <= 5000),
  aes(as.factor(coverage), value, fill = method))
p = p + geom_boxplot()
# p = p + facet_wrap(~n_cells)
save_plot('img/all_methods_estimated_fdr_05.pdf', p)

# now increase the fraction of effects
...
p = p + geom_boxplot()
# p = p + facet_wrap(~n_cells)
save_plot('img/all_methods_estimated_fdr_10_p25.pdf', p)
...
tmp = process_maude_results(metadata_em[1, ])
...
metadata = dplyr::mutate(metadata,
  maude_filename = paste0(sub('nimble', 'maude', dirname(filename)), '/',
    sub('.rds', '.csv', basename(filename))))
metadata = dplyr::mutate(metadata,
  mageck_filename = nimble_fname_to_mageck(filename))

sum(file.exists(metadata$mageck_filename))
sum(file.exists(metadata$maude_filename))
...
process_subgroup = function(metadata, p_gene_effects, n_total_genes, bin_configuration,
  ...
  # s_res = group_by(res, effect_size_type, n_guides_per_target, coverage_per_gene, rank)
  grouping_sym = ensyms(...)
  # print(grouping_sym)
  s_res = group_by(res, !!!grouping_sym)
  s_res = summarize(s_res, mean_fdr = mean(fdr), sd_fdr = sd(fdr),
    mean_sensitivity = mean(sensitivity), sd_sensitivity = sd(sensitivity),
    mean_estimated_fdr = mean(value), sd_estimated_fdr = sd(value))
  s_res = mutate(s_res, method = 'waterbear')
  ...
  if (!waterbear_only) {
    ...
    s_mageck_res = group_by(mageck_results, !!!grouping_sym)
  # s_res = group_by(res, !!!grouping_sym)
    s_mageck_res = summarize(s_mageck_res, mean_fdr = mean(fdr), sd_fdr = sd(fdr),
      mean_sensitivity = mean(sensitivity), sd_sensitivity = sd(sensitivity),
      mean_estimated_fdr = mean(value), sd_estimated_fdr = sd(value))
    ...
    # s_maude_res = group_by(maude_results, effect_size_type, n_cells, rank)
    s_maude_res = group_by(maude_results, !!!grouping_sym)
    s_maude_res = summarize(s_maude_res, mean_fdr = mean(fdr), sd_fdr = sd(fdr),
      mean_sensitivity = mean(sensitivity), sd_sensitivity = sd(sensitivity),
      mean_estimated_fdr = mean(value), sd_estimated_fdr = sd(value))
    s_maude_res = mutate(s_maude_res,
...
# tmp = process_subgroup(metadata, '0.10', '1000', '1', '3',
#   debug = FALSE, waterbear_only = FALSE,
#   # moi, effect_size_type, n_guides_per_target, coverage_per_gene, rank)
#   moi, n_cells, effect_size_type, n_guides_per_target, rank)

# testing posterior summary

# tmp = readRDS(sub('summary', 'posterior_summary', metadata_em$summary_filename[1]))
