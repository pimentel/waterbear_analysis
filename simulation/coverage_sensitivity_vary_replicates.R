...
p_10_2_replicates = process_subgroup(metadata, p_gene_effects, n_total_genes,
  bin_configuration, n_replicates, waterbear_only = TRUE)


p_10_efdr = get_estimated_fdr(p_10)

base = paste0('compare_coverage_', p_gene_effects, '_', n_total_genes, '_',
  bin_configuration, '_')


tmp_10_2 = dplyr::filter(p_10_2_replicates, effect_size_type == 'equalmixture')
# tmp_10_efdr = dplyr::filter(p_10_efdr, effect_size_type == 'equalmixture')

tmp_10_3 = dplyr::filter(p_10_3_replicates, effect_size_type == 'equalmixture')
# tmp_10_efdr = dplyr::filter(p_10_efdr, effect_size_type == 'equalmixture')
...
tmp_10 = bind_rows(
  dplyr::mutate(tmp_10_2, n_replicates = 2),
  dplyr::mutate(tmp_10_3, n_replicates = 3))
tmp_10 = dplyr::mutate(tmp_10, n_replicates = as.factor(n_replicates))
